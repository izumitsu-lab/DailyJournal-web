...
async function saveNotebookData() {
    ...
    if (typeof cloudIsSignedIn === 'function' && cloudIsSignedIn()) {
        if (typeof scheduleCloudSave === 'function') scheduleCloudSave(350);
    }
}
...
function saveAppTypes() {
    ...
    if (typeof cloudIsSignedIn === 'function' && cloudIsSignedIn() && typeof scheduleCloudSave === 'function') scheduleCloudSave();
}
...
window.onload = async () => {
    ...
    // Supabase設定済み・ログイン済みなら、この端末のローカルデータを確認した上で同期
    if (typeof initCloudSync === 'function') {
        await initCloudSync();
        if (typeof cloudIsSignedIn === 'function' && cloudIsSignedIn() && typeof handleCloudLoginAfterAuth === 'function') {
            await handleCloudLoginAfterAuth();
        }
    }
