...
(async () => {
  ...
  const nav = await p.evaluate(() => {
    const items = Array.from(document.querySelectorAll('.sidebar-scope-list .view-scope-item'));
    ...
    const modal = Array.from(document.querySelectorAll('#viewScopeModalList .view-scope-item')).slice(0, 5).map(i => i.querySelector('.view-scope-eyebrow').textContent + '/' + i.querySelector('.view-scope-name').textContent);
    return { names: items.map(i => i.querySelector('.view-scope-eyebrow').textContent + '/' + i.querySelector('.view-scope-name').textContent), modal, listW: Math.round(list.width), jumpW: Math.round(jump.width), belowCal: list.top >= cal.bottom, tops: items.map(i => Math.round(i.getBoundingClientRect().top)), sel: items.filter(i => i.classList.contains('selected')).map(i => i.id), radius: getComputedStyle(items[0]).borderRadius, icon: Math.round(items[0].querySelector('.view-scope-icon-box').getBoundingClientRect().width) };
  });
  check(nav.names.join() === nav.modal.join() && nav.names[0] === 'DAILY JOURNAL/DAILY', '下メニューの「表示ビューの切り替え」と同じ項目: ' + nav.names.join(' '));
  check(nav.radius === '16px' && nav.icon === 38, '同じデザイン（角丸・アイコン枠、サイドバー用に少しだけ小さく）');
  check(nav.belowCal && nav.tops.every((t, i) => i === 0 || t > nav.tops[i - 1]), 'カレンダーの下に縦一列');
  check(Math.abs(nav.listW - nav.jumpW) <= 1, `幅は「今日に戻る」と同じ横いっぱい (${nav.listW} / ${nav.jumpW})`);
  check(nav.sel.join() === 'btnScopeDay', 'DAILY が選択中（✓と青）');
  ...
  const pc = await p.evaluate(() => ({ on: showPinnedList, sel: document.getElementById('btnScopePins').classList.contains('selected'), week: document.getElementById('btnScopeWeek').classList.contains('selected'), check: getComputedStyle(document.querySelector('#btnScopePins .view-scope-check')).opacity }));
  check(pc.on && pc.sel && !pc.week && pc.check === '1', 'しおりを選ぶと、しおりに✓');
