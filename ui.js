...
function openModal(id) { 
    if (id === 'settingsModal') { 
        ...
        if (typeof updateCloudSettingsUI === 'function') updateCloudSettingsUI();
    } 
...
async function saveNewLog() {
    ...
    const newJournalId = (window.window.crypto?.randomUUID) ? `jl_${crypto.randomUUID()}` : `jl_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
async function saveEditedLog() {
    ...
    if (!journalData[d][i].id) {
        journalData[d][i].id = (window.window.crypto?.randomUUID) ? `jl_${crypto.randomUUID()}` : `jl_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    }
    journalData[d][i].updatedAt = new Date().toISOString();
    if (!journalData[d][i].createdAt) journalData[d][i].createdAt = journalData[d][i].updatedAt;
